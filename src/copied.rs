...
macro_rules! make_cloned_copied {
    ($Name:ident, $Trait:ident, $conv:ident, $verb:ident) => {
        ...
        impl<'this, Seq, Item> SequenceItem<'this> for $Name<Seq, Item> {
            type Item = Item;
        }
        ...
        impl<Seq, Item> IndexableSequence for $Name<Seq, Item>
        where
            Seq: IndexableSequence + for<'a> SequenceItem<'a, Item = &'a Item>,
            Item: $Trait,
        {
            #[inline]
        ...
        impl<'this, Seq, Item> SequenceIter<'this> for $Name<Seq, Item>
        where
            Seq: SequenceItem<'this, Item = &'this Item> + SequenceIter<'this>,
            Item: $Trait,
        {
            type Iter = iter::$Name<Seq::Iter>;
        }

        impl<Seq, Item> IterableSequence for $Name<Seq, Item>
        where
            Seq: IterableSequence + for<'a> SequenceItem<'a, Item = &'a Item>,
            Item: $Trait,
        {
            #[inline]
            fn iter(&self) -> iter::$Name<<Seq as SequenceIter<'_>>::Iter> {
                self.0.iter().$conv()
            }
        }
    };
